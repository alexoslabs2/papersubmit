# AGENTS.md — Paper Submission Platform

This file gives implementation agents the project-specific context, constraints, and quality gates required to work safely on the Paper Submission platform.

The source of truth is `PaperSubmission_Spec_v4.md`. When a conflict exists between generated code, previous plans, and the specification, follow the v4 specification.

---

## 1. Project Mission

Paper Submission is a lean, open-source Call for Papers (CFP) management platform for a single event.
...
Do not add a TLS step.

SMTP test must be attempted before completion. If SMTP is unreachable, warn the operator but allow proceeding.
...
1. Read this file and `PaperSubmission_Spec_v4.md` first.
